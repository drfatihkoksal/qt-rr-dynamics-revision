...
Across individualized and fixed hysteresis constants, the direct ischemic-minus-heart-rate-related mean-absolute estimate ranged from 1.64 to 4.87 ms, and each 95% CI included zero. The expanded 5–600-s grid estimate was 4.36 ms (95% CI, −0.88 to 9.59). Primary-grid tau selected the 10-s lower boundary in 71/190 leads and 300-s upper boundary in 15/190 leads; the expanded-grid analysis therefore assesses boundary restriction without optimizing on episode outcomes.

Alternative LTST protocols were not fully invariant. Direct estimates were 5.97 ms (95% CI, 1.48 to 10.45) under `.sta`, 4.87 ms (95% CI, −0.73 to 10.46) under primary `.stb`, and 6.36 ms (95% CI, 0.58 to 12.14) under `.stc`. The baseline contrasts remained positive. These analyses show threshold sensitivity and are not used to replace the prespecified primary result.

Rate-dynamic adjustment yielded 5.51 ms (95% CI, 2.92 to 8.10) for ischemic versus baseline, 2.23 ms (95% CI, −0.50 to 4.97) for heart-rate-related versus baseline, and 5.93 ms (95% CI, −0.33 to 12.20) for the direct contrast. The attenuation of the heart-rate-related association is consistent with residual dynamic-rate confounding.
...
All tables are CSV files in `paper/tables_revision/`. Every estimate is additionally indexed in `results_manifest.csv`, with source file and source script. Figure 1 source data are in `revision_work/figure_source/`; the remaining figures are generated directly from the frozen analysis CSV files by `src/revision_figures.py`.
